...
contract BEP1155Tradable is IBEP165, Ownable {
  ...
  function batchCreate(uint256[] memory _maxSupplies, uint256[] memory _prices)
    external onlyOwner
  {
    uint256 nCreate = _maxSupplies.length;
    require(nCreate == _prices.length, "BEP1155batchCreate: INVALID_ARRAYS_LENGTH");
    address sender = msg.sender;
    
    uint256 idCreate = _currentTokenID;
    _currentTokenID = _currentTokenID.add(nCreate);
    for (uint256 i = 0; i < nCreate; i++) {
      idCreate++;
      creators[idCreate] = sender;
      tokenMaxSupply[idCreate] = _maxSupplies[i];
      tokenPrice[idCreate] = _prices[i];
    }
  }
...
contract BEP1155 is BEP1155Tradable {
  using Strings for string;
  using Address for address;


  constructor() BEP1155Tradable("BEP1155 Token", "BEP1155", "https://RPC-URI.com/api/BEP1155/")  {
